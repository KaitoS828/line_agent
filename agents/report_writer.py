...
TOOLS = [
    ...
    {
        "name": "create_multi_report",
        "description": "複数トピックをまとめて調査し、Google Docs/PDFを並列で作成してURL一覧を返す",
        "input_schema": {
            "type": "object",
            "properties": {
                "topics": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "レポート化したいトピック一覧",
                },
                "folder_id": {"type": "string", "description": "保存先のGoogle DriveフォルダID（省略可）"},
                "max_workers": {"type": "integer", "description": "並列数（1〜5、デフォルト3）"},
            },
            "required": ["topics"],
        },
    },
...
- 「複数テーマを一気に」「まとめて何件か作って」→ create_multi_report（並列処理）
        ...
        try:
            match tool_name:
                ...
                case "create_multi_report":
                    if not self.drive_service:
                        return "❌ Google Drive未接続のため複数レポート作成できません"
                    return create_multi_report_pipeline(
                        self.drive_service,
                        tool_input["topics"],
                        tool_input.get("folder_id"),
                        tool_input.get("max_workers", 3),
                    )
                case _:
